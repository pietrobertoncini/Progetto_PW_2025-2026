...
// SICUREZZA: ACCESSO CONSENTITO A QUALSIASI UTENTE LOGGATO
...
$filtro_sala = isset($_GET['sala']) ? $_GET['sala'] : ''; // Formato: "id_settore|nome_sala"
...
if ($filtro_sala) {
    // CASO 1: VISTA GRIGLIA PER SINGOLA SALA
    $parts = explode('|', $filtro_sala);
    if (count($parts) === 2) {
        $id_sett_sel = (int)$parts[0];
        $nome_sala_sel = $parts[1];

        // Recuperiamo i dati estetici della sala selezionata
        ...
        // Recuperiamo le prenotazioni per la griglia
        $prenotazioni_griglia = getPrenotazioniGriglia($cid, $id_sett_sel, $nome_sala_sel, $lunedi_settimana, $domenica_settimana);
    }
} else {
    // CASO 2: VISTA LISTA GLOBALE (Nessuna sala selezionata)
    // Nota: Usiamo la funzione Admin per ottenere tutti i dati, ma li mostriamo in sola lettura
                                ...
                                <tr>
                                    ...
                                    <?php for ($i = 0; $i < 7; $i++):
                                        ...
                                        if (isset($prenotazioni_griglia[$data_curr][$ora])) {
                                            ...
                                            // STAMPA CELLA OCCUPATA (Solo all'inizio dell'evento)
                                            if ($cell['is_start']) {
                                                $durata = $info['durata'];
